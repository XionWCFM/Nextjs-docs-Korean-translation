...
- 트리(Tree) : 계층 구조를 시각하기 위한 규칙(convention)입니다. 부모 및 자식 컴포넌트가 있는 컴포넌트 트리, 폴더 구조 등이 트리의 예시 입니다.
...
디렉터리 간 라우트는 동일한 URL 경로로 해석되지 않아야하며 충돌을 방지하기 위해 build-time 오류가 발생할 수 있습니다.
